...
import { formatCurrency, accountTypeLabel } from '../../shared/utils';
...
export async function render(el: HTMLElement): Promise<void> {
  setTopbarActions(`
    <button class="btn btn-primary" id="btn-new-acc"><i class="ti ti-plus"></i> Nova conta</button>
  `);
  ...
  async function renderPage(): Promise<void> {
    ...
    el.innerHTML = `
      ...
      <div class="card" style="margin-bottom:20px">
        <div class="card-body" style="display:flex;align-items:center;justify-content:space-between;padding:20px 24px">
          ...
          <div style="display:flex;gap:40px">
            <div>
              <div style="font-size:11px;color:var(--text-3);margin-bottom:4px">Em conta</div>
            ...
            <div>
              ...
              <div style="font-size:18px;font-weight:600;color:var(--danger)">-${formatCurrency(debt)}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      ${accounts.length === 0
        ? `<div class="empty"><i class="ti ti-building-bank"></i>
            ...
            <p>Clique em "Nova conta" para começar.</p></div>`
        : `<div class="grid-2">
    ...
    el.querySelectorAll<HTMLElement>('[data-del-acc]').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!confirm('Excluir esta conta? Todas as transações serão removidas.')) return;
...
function accountCard(a: Account): string {
  const typeColors: Record<string, string> = {
    checking: '#8B5CF6', savings: '#1D9E75', credit_card: '#7F77DD', wallet: '#EF9F27',
  };
...
function openAccModal(acc: Account | null, onDone: () => void): void {
  openModal({
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <select class="form-ctrl" id="f-type">
            ${(['checking','savings','credit_card','wallet'] as AccountType[]).map(t =>
    ...
    onSave: async () => {
      ...
      if (!name) { alert('Informe o nome da conta.'); return false; }
