...
function isValidFinaBackup(filePath: string): boolean {
  ...
  if (header.toString('utf8') !== SQLITE_PLAINTEXT_HEADER) {
...
export function registerBackupHandlers(): void {
  ...
  ipcMain.handle('backup:import', async () => {
    ...
    // Se o backup restaurado estiver criptografado, não dá pra rodar
    ...
    fs.closeSync(fd);
    if (headerBuf.toString('utf8') === SQLITE_PLAINTEXT_HEADER) runMigrations();
