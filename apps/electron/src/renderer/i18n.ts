...
const entries: readonly Translation[] = [
  ...
  ['Sincronizar celular', 'Sync phone', 'Sincronizar celular', '同步手机'],
  ['Pareie o Fina Mobile e receba os lançamentos da fila', 'Pair Fina Mobile and receive the queued entries', 'Vincula Fina Mobile y recibe los movimientos de la cola', '配对 Fina Mobile 并接收队列中的记录'],
  ['Parear novo celular', 'Pair new phone', 'Vincular nuevo celular', '配对新手机'],
  ['Abra o Fina Mobile e escaneie o QR code abaixo. Um código vai aparecer no celular — digite esse código aqui para confirmar.', 'Open Fina Mobile and scan the QR code below. A code will appear on the phone — type that code here to confirm.', 'Abre Fina Mobile y escanea el código QR de abajo. Aparecerá un código en el celular: escríbelo aquí para confirmar.', '打开 Fina Mobile 并扫描下方的二维码。手机上会显示一个验证码——请在此处输入该验证码以确认。'],
  ['Gerar QR code', 'Generate QR code', 'Generar código QR', '生成二维码'],
  ['Aguardando celular', 'Waiting for phone', 'Esperando celular', '等待手机'],
  ['Conectado', 'Connected', 'Conectado', '已连接'],
  ['Aguardando o celular conectar...', 'Waiting for the phone to connect...', 'Esperando que el celular se conecte...', '等待手机连接……'],
  ['Celular conectado. Digite abaixo o código exibido nele.', 'Phone connected. Type below the code shown on it.', 'Celular conectado. Escribe abajo el código que aparece en él.', '手机已连接。请在下方输入其上显示的验证码。'],
  ['Nome do celular (ex: iPhone da Ana)', 'Phone name (e.g. Ana’s iPhone)', 'Nombre del celular (ej: iPhone de Ana)', '手机名称（例如：Ana 的 iPhone）'],
  ['Sem membro associado', 'No member associated', 'Sin miembro asociado', '未关联成员'],
  ['Código de 6 dígitos', '6-digit code', 'Código de 6 dígitos', '6 位验证码'],
  ['Celulares pareados', 'Paired phones', 'Celulares vinculados', '已配对的手机'],
  ['Nenhum celular pareado ainda.', 'No phones paired yet.', 'Aún no hay celulares vinculados.', '尚未配对任何手机。'],
  ['Membro', 'Member', 'Miembro', '成员'],
  ['Pareado em', 'Paired on', 'Vinculado el', '配对时间'],
  ['Último sync', 'Last sync', 'Última sincronización', '上次同步'],
  ['Revogado', 'Revoked', 'Revocado', '已撤销'],
  ['Revogar', 'Revoke', 'Revocar', '撤销'],
  ['Atividade recente', 'Recent activity', 'Actividad reciente', '近期活动'],
  ['O celular não confirmou o código a tempo.', 'The phone did not confirm the code in time.', 'El celular no confirmó el código a tiempo.', '手机未能及时确认验证码。'],
  ['Celular "{deviceName}" pareado com sucesso.', 'Phone "{deviceName}" paired successfully.', 'Celular "{deviceName}" vinculado con éxito.', '手机“{deviceName}”配对成功。'],
  ['{count} lançamento(s) recebido(s) do celular.', '{count} entry(ies) received from the phone.', '{count} movimiento(s) recibido(s) del celular.', '已从手机接收 {count} 条记录。'],
  ['Informe um nome para identificar o celular.', 'Enter a name to identify the phone.', 'Indica un nombre para identificar el celular.', '请输入用于识别该手机的名称。'],
  ['Revogar o acesso desse celular? Ele vai precisar ser pareado de novo para voltar a enviar lançamentos.', 'Revoke this phone’s access? It will need to be paired again to send entries.', '¿Revocar el acceso de este celular? Deberá vincularse de nuevo para volver a enviar movimientos.', '要撤销该手机的访问权限吗？之后需要重新配对才能再次发送记录。'],
];
