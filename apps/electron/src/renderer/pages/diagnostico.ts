...
export async function render(el: HTMLElement): Promise<void> {
  ...
  el.innerHTML = `
    ...
    <div class="grid-3" style="margin-bottom:20px">
      ...
      ${metricCard('Reserva estimada', td("{value} meses", [formatDecimal(reserveMonths)]), td("{value} em contas", [formatCurrency(liquidBalance)]), 'ti-shield', reserveMonths < 1 ? 'var(--danger)' : reserveMonths < 3 ? 'var(--warning)' : 'var(--accent)')}
