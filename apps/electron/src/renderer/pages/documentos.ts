...
export async function render(el: HTMLElement): Promise<void> {
  ...
  const draw = (): void => {
    el.innerHTML = `<div class="card" style="margin-bottom:20px"><div class="card-header"><i class="ti ti-lock"></i> ${tk('documents.vault.title')}</div><div class="card-hr"></div><div class="card-body"><p style="color:var(--text-2);font-size:.85rem">${tk('documents.vault.description')}</p><button class="btn btn-primary" id="document-import"><i class="ti ti-upload"></i> ${tk('documents.import')}</button></div></div>
      <div class="card"><div class="card-header">${tpk('documents.count.one', 'documents.count.other', documents.length)}</div><div class="card-hr"></div><div class="card-body">${documents.length ? `<div style="display:flex;flex-direction:column;gap:8px">${documents.map(documentRow).join('')}</div>` : `<div class="empty"><i class="ti ti-folder-off"></i><div class="empty-title">${tk('documents.empty.title')}</div><p>${tk('documents.empty.description')}</p></div>`}</div></div>`;
    ...
    el.querySelectorAll<HTMLElement>('[data-delete-document]').forEach(button => button.addEventListener('click', async () => {
      if (!window.confirm(tk('documents.remove.confirm'))) return;
...
function documentRow(doc: FinancialDocument): string {
  const size = doc.size_bytes < 1024 * 1024 ? `${Math.max(1, Math.round(doc.size_bytes / 1024))} KB` : `${formatDecimal(doc.size_bytes / 1024 / 1024)} MB`;
