...
# recente do Fina e instala com o gerenciador de pacotes da distro. Cobre
# openSUSE Leap e Fedora (mesmo RPM genérico do electron-builder, sem specs
# por distro) e Ubuntu/Debian (.deb), ambos gerados por
# .github/workflows/release.yml. Em Arch use o pacote no AUR (`yay -S fina`),
# que já existe e é o caminho recomendado — não há RPM/.deb equivalente
# mantido aqui pra Arch.
#
# Uso:
#   curl -fsSL https://raw.githubusercontent.com/britors/Fina/main/scripts/install.sh | sudo bash
#
# FINA_VERSION=v17.2.0 sudo -E bash install.sh   # trava numa tag específica
set -euo pipefail
...
download_release_assets() {
  local suffix="$1"
  local release_tag="${FINA_VERSION:-latest}"
  local api_url
...
case "$distro_id $distro_id_like" in
  *arch*)
    cat >&2 <<'EOF'
Detectei Arch. Este instalador só baixa RPM/.deb pra openSUSE/Fedora/
Ubuntu; em Arch use o pacote do AUR, que já é o caminho suportado:

  yay -S fina

(ou `paru -S fina`, se preferir).
EOF
    exit 1
    ;;
  *opensuse*|*suse*)
  ...
  *)
    ...
    echo "" >&2
    echo "Alternativas:" >&2
    echo "- Arch: pacote no AUR (yay -S fina)" >&2
    echo "- Outras distros: baixe o .deb ou .rpm manualmente e adapte o comando" >&2
    echo "  do gerenciador de pacotes da sua distro:" >&2
    echo "  https://github.com/$REPO/releases" >&2
    exit 1
    ;;
esac
...
- App: /usr/bin/fina (ou pelo atalho "Fina" no menu)

Empacotamento ainda é considerado de teste — reporte problemas em
https://github.com/britors/Fina/issues.
EOF
